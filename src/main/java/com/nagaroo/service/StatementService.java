    ...
    public List<Statement> getFilteredStatement(Filter filter){
        ...
        return statements.stream().filter(statement -> {
            ...
            if(Objects.nonNull(filter.getFromdateField()) && Objects.nonNull(filter.getTodateField()) ){
                ...
                if(LocalDate.parse(filter.getFromdateField(), StatementConstants.formatter1).isAfter(statementDate)
                        || LocalDate.parse(filter.getTodateField(),StatementConstants.formatter1 ).isBefore(statementDate)){
                    return false;
                }
            }
